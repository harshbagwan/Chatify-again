import express from "express";
// import dotenv from "dotenv";
import cookieParser from "cookie-parser";
import path from "path";
import cors from "cors";
import authRoutes from "./routes/auth.route.js";// make sure to write .js extension since we are importing a local file from our codebase
import messageRoutes from "./routes/message.route.js";
import { connectDB } from "./lib/db.js";
import { ENV } from "./lib/env.js";
// dotenv.config();1

const app = express();
const __dirname = path.resolve();

const PORT = ENV.PORT || 3000;

app.use(express.json()); // req.body
app.use(cors({origin:ENV.CLIENT_URL,credentials:true})); // allow cross-origin requests from the frontend client URL and allow credentials (cookies) to be sent with the requests
app.use(cookieParser()); // req.cookies

app.use("/api/auth",authRoutes);  // prefix them with /api/auth
app.use("/api/messages",messageRoutes);

if(ENV.NODE_ENV === "production"){
    app.use(express.static(path.join(__dirname,"../frontend/dist")));
    app.get("*",(_,res)=>{
        res.sendFile(path.join(__dirname,"../frontend/dist/index.html"));
    });
}

app.listen(PORT,()=>{
    console.log("Server running on port :" + PORT )
    connectDB()
});


